    ...
    <div className="reviews-container">
      ...
      <ul className='reviews-card-layout'>
        {reviewData.map((card, index) => {
          return (
            <li key={card.id} tabIndex={0} aria-label='A review card' className="review-card">
          <div className="upper-container">
            <div className="pp-container">
              <img src={profile[index]} alt="Profile pic" />
            </div>
            <div className="ratings-container">
              <img src={stars[index]} alt="Ratings, five stars" />
            </div>
          </div>
          <article>
            <h4><span className="user-highlight">{card.userName}</span> has left a review.</h4>
